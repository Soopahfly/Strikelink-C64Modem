...
static unsigned char petToAscTable[256] = {
    ...
    0xd8, 0xd9, 0xda, 0xdb, 0xdc, 0xdd, 0xde, 0xdf, 0x80, 0x81, 0x82, 0x83,
    0x84, 0x85, 0x86, 0x87, 0x88, 0x89, 0x8a, 0x8b, 0x8c, 0x8d, 0x8e, 0x8f,
    0x90, 0x91, 0x92, 0x0c, 0x94, 0x95, 0x96, 0x97, 0x98, 0x99, 0x9a, 0x9b,
    0x9c, 0x9d, 0x9e, 0x9f, 0xa0, 0xa1, 0xa2, 0xa3, 0xa4, 0xa5, 0xa6, 0xa7,
    0xa8, 0xa9, 0xaa, 0xab, 0xac, 0xad, 0xae, 0xaf, 0xb0, 0xb1, 0xb2, 0xb3,
    0xb4, 0xb5, 0xb6, 0xb7, 0xb8, 0xb9, 0xba, 0xbb, 0xbc, 0xbd, 0xbe, 0xbf,
    0x60, 0x41, 0x42, 0x43, 0x44, 0x45, 0x46, 0x47, 0x48, 0x49, 0x4a, 0x4b,
    0x4c, 0x4d, 0x4e, 0x4f, 0x50, 0x51, 0x52, 0x53, 0x54, 0x55, 0x56, 0x57,
    0x58, 0x59, 0x5a, 0x7b, 0x7c, 0x7d, 0x7e, 0x7f, 0xa0, 0xa1, 0xa2, 0xa3,
...
// #define DEBUG 1          // Print additional debug information to serial
// channel
#undef DEBUG
#define LISTEN_PORT                                                            \
  6400 // Listen to this if not connected. Set to zero to disable.
int tcpServerPort = LISTEN_PORT;
#define RING_INTERVAL                                                          \
  3000 // How often to print RING when having a new incoming connection (ms)
...
#define TX_BUF_SIZE                                                            \
  256 // Buffer where to read from serial before writing to TCP
// (that direction is very blocking by the ESP TCP stack,
// so we can't do one byte a time.)
uint8_t txBuf[TX_BUF_SIZE];
const int speedDialAddresses[] = {
    DIAL0_ADDRESS, DIAL1_ADDRESS, DIAL2_ADDRESS, DIAL3_ADDRESS, DIAL4_ADDRESS,
    DIAL5_ADDRESS, DIAL6_ADDRESS, DIAL7_ADDRESS, DIAL8_ADDRESS, DIAL9_ADDRESS};
...
byte ringCount = 0;
String resultCodes[] = {"OK", "CONNECT",     "RING", "NO CARRIER", "ERROR",
...
bool petTranslate =
    false; // Fix PET MCTerm 1.26C Pet->ASCII encoding to actual ASCII
bool hex = false;
...
void readSettings() {
  ...
  // EEPROM SSID and password write routine bypassed to hard-set them in code
  // instead, trying to resolve connectivity problems (CJM-11/24/2023)
  ...
  tcpServerPort = word(EEPROM.read(SERVER_PORT_ADDRESS),
                       EEPROM.read(SERVER_PORT_ADDRESS + 1));
...
void defaultEEPROM() {
  ...
  setEEPROM("YOUR_SSID", SSID_ADDRESS, SSID_LEN);
  setEEPROM("YOUR_PASS", PASS_ADDRESS, PASS_LEN);
  ...
  setEEPROM("SYSTEM BUSY. TRY AGAIN LATER.", BUSY_MSG_ADDRESS, BUSY_MSG_LEN);
  EEPROM.commit();
}
...
void setEEPROM(String inString, int startAddress, int maxLen) {
  for (int i = startAddress; i < inString.length() + startAddress; i++) {
    ...
    // Serial.print(i, DEC); Serial.print(": "); Serial.println(inString[i -
    // startAddress]); if (EEPROM.read(i) != inString[i - startAddress]) {
    // Serial.print(" (!)"); } Serial.println();
  }
  // null pad the remainder of the memory space
  for (int i = inString.length() + startAddress; i < maxLen + startAddress;
       i++) {
    EEPROM.write(i, 0x00);
...
void sendResult(int resultCode) {
  ...
  if (resultCode == R_CONNECT) {
    Serial.print(String(resultCodes[R_CONNECT]) + F(" ") +
                 String(bauds[serialspeed]));
  } else if (resultCode == R_NOCARRIER) {
    Serial.print(String(resultCodes[R_NOCARRIER]) + F(" (") +
                 connectTimeString() + F(")"));
  } else {
...
void connectWiFi() {
  if (ssid == "" || password == "") {
    Serial.println(
        F("NEED TO CONFIGURE SSID AND PASSWORD. TYPE AT? FOR HELP."));
    return;
  }

  // ### ADDED CJM-11/24/2023
  ...
  Serial.println();
  if (i == 61) { // one more tick than the value set in Wifi.status above
...
void setBaudRate(int inSpeed) {
  ...
  Serial.print(F("SWITCHING SERIAL PORT TO "));
...
void displayNetworkStatus() {
  ...
  yield();
  Serial.print(F("WEB CONFIG.: HTTP://"));
  Serial.println(WiFi.localIP());
  yield();
...
void displayStoredSettings() {
  ...
  // Serial.println(word(EEPROM.read(SERVER_PORT_ADDRESS),
  // EEPROM.read(SERVER_PORT_ADDRESS+1))); yield();
  Serial.print(F("BUSY MSG: "));
  ...
  Serial.println(F("STORED SPEED DIAL:"));
  for (int i = 0; i < 10; i++) {
    Serial.print(i);
    Serial.print(F(": "));
...
void displayHelp() {
  welcome();
  Serial.println(F("AT COMMAND SUMMARY:"));
  yield();
  Serial.println(F("DIAL HOST.....: ATDTHOST:PORT"));
  yield();
  Serial.println(F("SPEED DIAL....: ATDSN (N=0-9)"));
  yield();
  Serial.println(F("SET SPEED DIAL: AT&ZN=HOST:PORT (N=0-9)"));
  yield();
  Serial.println(F("HANDLE TELNET.: ATNETN (N=0,1)"));
  yield();
  Serial.println(F("PET MCTERM TR.: ATPETN (N=0,1)"));
  yield();
  Serial.println(F("NETWORK INFO..: ATI"));
  yield();
  Serial.println(F("HTTP GET......: ATGET<URL>"));
  yield();
  // Serial.println("SERVER PORT...: AT$SP=N (N=1-65535)"); yield();
  Serial.println(F("AUTO ANSWER...: ATS0=N (N=0,1)"));
  yield();
  Serial.println(F("SET BUSY MSG..: AT$BM=YOUR BUSY MESSAGE"));
  yield();
  Serial.println(F("LOAD NVRAM....: ATZ"));
  yield();
  Serial.println(F("SAVE TO NVRAM.: AT&W"));
  yield();
  Serial.println(F("SHOW SETTINGS.: AT&V"));
  yield();
  Serial.println(F("FACT. DEFAULTS: AT&F"));
  yield();
  Serial.println(F("PIN POLARITY..: AT&PN (N=0/INV,1/NORM)"));
  yield();
  Serial.println(F("ECHO OFF/ON...: ATE0 / ATE1"));
  yield();
  Serial.println(F("VERBOSE OFF/ON: ATV0 / ATV1"));
  yield();
  Serial.println(F("SET SSID......: AT$SSID=WIFISSID"));
  yield();
  Serial.println(F("SET PASSWORD..: AT$PASS=WIFIPASSWORD"));
  yield();
  Serial.println(F("SET BAUD RATE.: AT$SB=N (3,12,24,48,96"));
  yield();
  Serial.println(F("                192,384,576,1152)*100"));
  yield();
  waitForSpace();
  Serial.println(F("FLOW CONTROL..: AT&KN (N=0/N,1/HW,2/SW)"));
  yield();
  Serial.println(F("WIFI OFF/ON...: ATC0 / ATC1"));
  yield();
  Serial.println(F("HANGUP........: ATH"));
  yield();
  Serial.println(F("ENTER CMD MODE: +++"));
  yield();
  Serial.println(F("EXIT CMD MODE.: ATO"));
  yield();
  Serial.println(F("QUERY MOST COMMANDS FOLLOWED BY '?'"));
  yield();
}
...
void storeSpeedDial(byte num, String location) {
  ...
  // Serial.print("STORED "); Serial.print(num); Serial.print(": ");
...
void welcome() {
  ...
  Serial.println(
      F("MODIFIED BY SOOPAHFLY - 2026 - KEEPING UP WITH COMMODORE!"));
...
void setup() {
  ...
  if (EEPROM.read(VERSION_ADDRESS) != VERSIONA ||
      EEPROM.read(VERSION_ADDRESS + 1) != VERSIONB) {
    defaultEEPROM();
  }
  ...
  // while (c != 8 && c != 127 && c!= 20) { // Check for the backspace key to
  // begin while (c != 32) { // Check for space to begin
  while (c != 0x0a && c != 0x0d) {
    if (Serial.available() > 0) {
      c = Serial.read();
      if (petTranslate == true) {
  ...
  connectWiFi();
  sendResult(R_OK);
  // tcpServer(tcpServerPort); // can't start tcpServer inside a function-- must
  // live outside

  digitalWrite(LED_PIN, LOW); // on

  webServer.on(F("/"), handleRoot);
  webServer.on(F("/ath"), handleWebHangUp);
  webServer.begin();
  mdns.begin(F("C64WiFi"), WiFi.localIP());
}
...
void handleWebHangUp() {
  String t = F("NO CARRIER (") + connectTimeString() + F(")");
  hangUp();
  webServer.send(200, F("text/plain"), t);
}

void handleRoot() {
  String page = F("WIFI STATUS: ");
  if (WiFi.status() == WL_CONNECTED) {
    page.concat(F("CONNECTED"));
  }
  if (WiFi.status() == WL_IDLE_STATUS) {
    page.concat(F("OFFLINE"));
  }
  if (WiFi.status() == WL_CONNECT_FAILED) {
    page.concat(F("CONNECT FAILED"));
  }
  if (WiFi.status() == WL_NO_SSID_AVAIL) {
    page.concat(F("SSID UNAVAILABLE"));
  }
  if (WiFi.status() == WL_CONNECTION_LOST) {
    page.concat(F("CONNECTION LOST"));
  }
  if (WiFi.status() == WL_DISCONNECTED) {
    page.concat(F("DISCONNECTED"));
  }
  if (WiFi.status() == WL_SCAN_COMPLETED) {
    page.concat(F("SCAN COMPLETED"));
  }
  yield();
  page.concat(F("\nSSID.......: ") + WiFi.SSID());
  ...
  page.concat(F("\nCALL STATUS: "));
  if (callConnected) {
    page.concat(F("CONNECTED TO "));
    page.concat(ipToString(tcpClient.remoteIP()));
    page.concat(F("\nCALL LENGTH: "));
    page.concat(connectTimeString());
    yield();
  } else {
    page.concat(F("NOT CONNECTED"));
  }
  page.concat(F("\n"));
  webServer.send(200, F("text/plain"), page);
  delay(100);
}

/**
   Turn on the LED and store the time, so the LED will be shortly after turned
   off
*/
void led_on() {
  digitalWrite(LED_PIN, !digitalRead(LED_PIN));
...
void handleIncomingConnection() {
  if (callConnected == 1 || (autoAnswer == false && ringCount > 3)) {
    ...
    anotherClient.print(F("\r\n"));
    anotherClient.print(F("CURRENT CALL LENGTH: "));
  ...
  if (autoAnswer == true) {
    WiFiClient tempClient =
        tcpServer.available(); // this is the key to keeping the connection open
    tcpClient = tempClient; // hand over the new connection to the global client
    tempClient.stop();      // stop the temporary one
    sendString(String(F("RING ")) + ipToString(tcpClient.remoteIP()));
    delay(1000);
    sendResult(R_CONNECT);
    connectTime = millis();
    cmdMode = false;
...
void dialOut(String upCmd) {
  ...
  if (upCmd.indexOf(F("ATDS")) == 0) {
    byte speedNum = upCmd.substring(4, 5).toInt();
    portIndex = speedDials[speedNum].indexOf(F(":"));
    if (portIndex != -1) {
...
void command() {
  ...
  /**** Dial to host ****/
  else if ((upCmd.indexOf(F("ATDT")) == 0) || (upCmd.indexOf(F("ATDP")) == 0) ||
           (upCmd.indexOf(F("ATDI")) == 0) || (upCmd.indexOf(F("ATDS")) == 0)) {
    dialOut(upCmd);
  }

  /**** Change telnet mode ****/
  else if (upCmd == F("ATNET0")) {
    telnet = false;
    sendResult(R_OK);
  } else if (upCmd == F("ATNET1")) {
    telnet = true;
    sendResult(R_OK);
  }

  else if (upCmd == F("ATNET?")) {
    Serial.println(String(telnet));
    sendResult(R_OK);
  }

  /**** Answer to incoming connection ****/
  else if ((upCmd == F("ATA")) && tcpServer.hasClient()) {
    answerCall();
  }

  /**** Display Help ****/
  else if (upCmd == F("AT?") || upCmd == F("ATHELP")) {
    displayHelp();
    sendResult(R_OK);
  }
  ...
  /**** Disconnect WiFi ****/
  else if (upCmd == F("ATC0")) {
    disconnectWiFi();
    sendResult(R_OK);
  }

  /**** Connect WiFi ****/
  else if (upCmd == F("ATC1")) {
    connectWiFi();
    sendResult(R_OK);
  }

  /**** Control local echo in command mode ****/
  else if (upCmd.indexOf(F("ATE")) == 0) {
    if (upCmd.substring(3, 4) == F("?")) {
      sendString(String(echo));
      sendResult(R_OK);
    } else if (upCmd.substring(3, 4) == F("0")) {
      echo = 0;
      sendResult(R_OK);
    } else if (upCmd.substring(3, 4) == F("1")) {
      echo = 1;
      sendResult(R_OK);
    } else {
      sendResult(R_ERROR);
  ...
  else if (upCmd.indexOf(F("AT&P")) == 0) {
    if (upCmd.substring(4, 5) == F("?")) {
      sendString(String(pinPolarity));
      sendResult(R_OK);
    } else if (upCmd.substring(4, 5) == F("0")) {
      pinPolarity = P_INVERTED;
      sendResult(R_OK);
      setCarrier(callConnected);
    } else if (upCmd.substring(4, 5) == F("1")) {
  ...
  else if (upCmd.indexOf(F("AT&K")) == 0) {
    if (upCmd.substring(4, 5) == F("?")) {
      ...
      flowControl = 1;
      sendResult(R_OK);
    } else if (upCmd.substring(4, 5) == F("2")) {
      flowControl = 2;
      sendResult(R_OK);
    } else {
      sendResult(R_ERROR);
  ...
  else if (upCmd.indexOf(F("AT$SB=")) == 0) {
  ...
  /**** Set busy message ****/
  else if (upCmd.indexOf(F("AT$BM=")) == 0) {
  ...
  else if (upCmd == F("AT&W")) {
    writeSettings();
    sendResult(R_OK);
  }

  /**** Set or display a speed dial number ****/
  else if (upCmd.indexOf(F("AT&Z")) == 0) {
    byte speedNum = upCmd.substring(4, 5).toInt();
    if (speedNum >= 0 && speedNum <= 9) {
      if (upCmd.substring(5, 6) == F("=")) {
  ...
  /**** Set WiFi SSID ****/
  else if (upCmd.indexOf(F("AT$SSID=")) == 0) {
    ssid = cmd.substring(8);
    sendResult(R_OK);
  }

  /**** Display WiFi SSID ****/
  else if (upCmd == F("AT$SSID?")) {
    sendString(ssid);
    sendResult(R_OK);
  }

  /**** Set WiFi Password ****/
  else if (upCmd.indexOf(F("AT$PASS=")) == 0) {
    password = cmd.substring(8);
    sendResult(R_OK);
  }

  /**** Display WiFi Password ****/
  else if (upCmd == F("AT$PASS?")) {
    sendString(password);
    sendResult(R_OK);
  }

  /**** Reset EEPROM and current settings to factory defaults ****/
  else if (upCmd == F("AT&F")) {
    defaultEEPROM();
    readSettings();
    sendResult(R_OK);
  }
  ...
  /**** Set auto answer off ****/
  else if (upCmd == F("ATS0=0")) {
    autoAnswer = false;
    sendResult(R_OK);
  }

  /**** Set auto answer on ****/
  else if (upCmd == F("ATS0=1")) {
    autoAnswer = true;
    sendResult(R_OK);
  }

  /**** Display auto answer setting ****/
  else if (upCmd == F("ATS0?")) {
    sendString(String(autoAnswer));
    sendResult(R_OK);
  }

  /**** Set PET MCTerm Translate On ****/
  else if (upCmd == F("ATPET=1")) {
    petTranslate = true;
    sendResult(R_OK);
  }

  /**** Set PET MCTerm Translate Off ****/
  else if (upCmd == F("ATPET=0")) {
    petTranslate = false;
    sendResult(R_OK);
  }

  /**** Display PET MCTerm Translate Setting ****/
  else if (upCmd == F("ATPET?")) {
    sendString(String(petTranslate));
    sendResult(R_OK);
  }

  /**** Set HEX Translate On ****/
  else if (upCmd == F("ATHEX=1")) {
    hex = true;
    sendResult(R_OK);
  }

  /**** Set HEX Translate Off ****/
  else if (upCmd == F("ATHEX=0")) {
    hex = false;
    sendResult(R_OK);
  }

  /**** Hang up a call ****/
  else if (upCmd.indexOf(F("ATH")) == 0) {
    hangUp();
  }

  /**** Hang up a call ****/
  else if (upCmd.indexOf(F("AT$RB")) == 0) {
    sendResult(R_OK);
    Serial.flush();
    delay(500);
    ESP.reset();
  ...
  else if (upCmd.indexOf(F("AT$SP=")) == 0) {
    tcpServerPort = upCmd.substring(6).toInt();
    sendString(F("CHANGES REQUIRES NV SAVE (AT&W) AND RESTART"));
    sendResult(R_OK);
  }

  /**** Display icoming TCP server port ****/
  else if (upCmd == F("AT$SP?")) {
    sendString(String(tcpServerPort));
    sendResult(R_OK);
  }

  /**** See my IP address ****/
  else if (upCmd == F("ATIP?")) {
    Serial.println(WiFi.localIP());
    sendResult(R_OK);
  }

  /**** HTTP GET request ****/
  else if (upCmd.indexOf(F("ATGET")) == 0) {
    // From the URL, aquire required variables
    // (12 = "ATGEThttp://")
    int portIndex =
        cmd.indexOf(F(":"), 12); // Index where port number might begin
    int pathIndex = cmd.indexOf(
        F("/"),
        12); // Index first host name and possible port ends and path begins
    int port;
    String path, host;
    if (pathIndex < 0) {
...
// RTS/CTS protocol is a method of handshaking which uses one wire in each
// direction to allow each device to indicate to the other whether or not it is
// ready to receive data at any given moment. One device sends on RTS and
// listens on CTS; the other does the reverse. A device should drive its
// handshake-output wire low when it is ready to receive data, and high when it
// is not. A device that wishes to send data should not start sending any bytes
// while the handshake-input wire is low; if it sees the handshake wire go high,
// it should finish transmitting the current byte and then wait for the
// handshake wire to go low before transmitting any more.
...
void loop() {
  ...
  if (cmdMode == true) {
    ...
    // In command mode - don't exchange with TCP but gather characters to a
    // string
    if (Serial.available()) {
      ...
      if (petTranslate == true)
        ...
        if (chr > 127)
          ...
          // Convert uppercase PETSCII to lowercase ASCII (C64) in command mode
          // only
          if ((chr >= 193) && (chr <= 218))
            chr -= 96;

      // Return, enter, new line, carriage return.. anything goes to end the
      // command
      if ((chr == '\n') || (chr == '\r')) {
        command();
      }
    ...
    while (tcpClient.available() && txPaused == false) {
      ...
      if ((telnet == true) && (rxByte == 0xff)) {
        ...
        if (rxByte == 0xff) {
          ...
          // rxByte has now the first byte of the actual non-escaped control
          // code
#ifdef DEBUG
          Serial.print(rxByte);
          Serial.print(",");
          ...
          // rxByte has now the second byte of the actual non-escaped control
          // code
#ifdef DEBUG
          Serial.print(rxByte);
          Serial.flush();
